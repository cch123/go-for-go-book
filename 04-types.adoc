...
`ast.Object` における `Kind` フィールドに対応するものを持たないため、type switch でその種類を判別することになります。型におけるオブジェクト（TODO: 表現）と、それを表すデータ型は以下のようになっています。
...
組み込み関数を表す構造体です。組み込み関数は決まった型を持たないため、`Type()` は invalid な型を返します。
...
といったことは（ただちには）知ることができません。そこで `Check` 関数の最後の引数に渡す構造体、`types.Info` の出番となります。

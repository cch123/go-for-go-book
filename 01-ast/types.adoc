...
以下でこれらサブインターフェースと、その実装のうち主要なものを見ていきます。
...
NOTE: `FieldList` はその名の通り複数の組を表すこともできますが、Goの文法上、レシーバとしてはただ1つの組のみが有効です。`FieldList` が構造体のフィールド宣言の表現などにも使われるためこのような構造になっています。

[WARNING]
====
ただし、`go/parser` は複数の組からなるレシーバをエラーなく解析します！ コメントによると、シンプルさとロバスト性のためだということです。
...
`Specs` フィールドはその要素が `ast.Spec` インタフェースであるスライスと宣言されていますが、要素の具体的な型はバラバラではなく、`Tok` フィールドによってひとつの型に決まります。

...
HTTPリクエストにしたがってパッケージのドキュメントを返すのが `getDoc`（gosource:gddo-server/main.go#L74[717c3ed,gddo]）です。リクエストされたパッケージは `crawlDoc` から `github.com/golang/gddo/doc.Get` を経由して呼び出される `github.com/golang/gddo/gosrc.Get` によって、そのパッケージがホストされているリモートのVCSから取得されます。

* `getStatic`
  * "`getStatic gets a diretory from a statically known service`"
  * github とか。gosrc/github.go
  * Directory を得る newPackage
* `getDynamic`
  * getVCSDir（vcs.go）

=== 外部サービスへの対応

GitHubやBitBuckectなど有名どころでAPIも提供されているサービスに対しては、それぞれからソースコードを取得する処理が実装されています。
...
`Presentation` はプレゼンテーション用の機能で、http://go-talks.appspot.com/[go-talks.appspot.org]でのみ利用されているものです。
